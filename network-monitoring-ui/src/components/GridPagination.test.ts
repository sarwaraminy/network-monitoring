...
describe('describePageRange', () => {
  it('reads as the source system does', () => {
    expect(describePageRange(facts())).toBe('1–25 of 100');
  });

  it('follows the page', () => {
    expect(describePageRange(facts({ page: 3 }))).toBe('51–75 of 100');
  });

  it('does not run the last page past the end', () => {
    // 90 rows in pages of 25: the fourth holds 15, not 25.
    expect(describePageRange(facts({ total: 90, unfiltered: 90, page: 4 }))).toBe('76–90 of 90');
  });

  it('starts at zero when there is nothing', () => {
    // "1–0 of 0" is the arithmetic answer and a nonsense sentence.
    expect(describePageRange(facts({ total: 0, unfiltered: 0, pageCount: 1 }))).toBe('0–0 of 0');
  });
  ...
  it('says how much a filter is hiding', () => {
    ...
    expect(describePageRange(facts({ total: 12, unfiltered: 340, pageCount: 1 }))).toBe(
      '1–12 of 12, filtered from 340',
    );
  });

  it('says nothing about filtering when nothing is filtered', () => {
    expect(describePageRange(facts({ total: 100, unfiltered: 100 }))).not.toMatch(/filtered/);
  });

  it('groups thousands, so a large count is readable at a glance', () => {
    expect(describePageRange(facts({ total: 12_450, unfiltered: 12_450, page: 101, pageCount: 498 }))).toBe(
      '2,501–2,525 of 12,450',
    );
  });
});
