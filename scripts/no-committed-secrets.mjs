...
/**
 * Values that are obviously a stand-in rather than a credential.
 *
 * Added the first time this hook refused a commit it should not have: the README
 * documents the connection string with `yourpassword` in it, and
 * `auth-rejection.test.ts` points at a deliberately unreachable database as
 * `nobody:nothing`. Both are exactly the shape this looks for and neither is a
 * secret.
 *
 ...
 * deployment.
 *
 * This is the failure mode the docblock above predicts for an entropy-guessing
 * scanner, arriving early and small. If this list ever has to grow much, that is
 * the signal the check has outlived its usefulness rather than a reason to keep
 * extending it.
 */
...
const SECRET_PATTERNS = [
  {
    ...
    ignore: (value) => PLACEHOLDERS.has(value.toLowerCase()),
  },
  {
