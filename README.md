...
| What | Where |
| --- | --- |
| **One three-layer settings resolver instead of three** — the roadmap's own written-down refactor, due because #63 had just written the third copy. `notify/settings.ts`, `services/adhoc-settings.ts` and `services/flow-settings.ts` each carried their own environment → stored row → default walk, and flow imported `SettingSource` out of adhoc because that was the nearest place it existed. **The layer walk is now shared and the parsing is not**, and that split is the finding rather than a compromise: the obvious extraction is one generic `parseFieldValue` with a `kind` switch, and the three parsers disagree on four points that are each deliberate, documented and load-bearing — whether a blank string is a value, whether an unrecognised boolean is `false` or unset (delivery's is legacy compatibility with `env.ts`'s old `bool()`, so `NOTIFY_INCLUDE_EVIDENCE=maybe` keeps meaning false rather than flipping every boolean that defaults on), whether integer bounds are enforced, and whether strings are trimmed (adhoc does not, because `ADHOC_DB_PASSWORD`'s whitespace is part of the password). Folding those together means four compatibility flags serving three callers. So each module keeps its parser and passes it in, and what is shared is the part that has actually broken: the order of the layers and what counts as the environment saying nothing — the rule `compose-unpinned.test.ts` and `env-defaults.test.ts` exist for, wrong in shipped Compose files twice and both `.env` examples once. The one per-field exception is named rather than buried: `clearedValue`, which is flow's `exporters` and nothing else — declared as the value clearing the field produces, typed to that field, so a table saying `[]` where the setting is a string is refused where it is written. Every move was green with its existing tests unedited, which was the acceptance test for each step | *this branch* |
...
1. **Finish internationalisation.** No engineering left — what remains cannot be done by
   ...
   **This is the only entry left.** Everything else the roadmap carried has shipped, which
   ...
   The settings-resolver refactor that stood here is done — see the table above.
